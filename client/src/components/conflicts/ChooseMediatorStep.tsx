...
const mockMediators: Mediator[] = [
  {
    id: "1",
    name: "Alice Johnson",
    expertise: ["Contracts", "Finance"],
    rating: 4.8,
    bio: "Experienced in contract disputes and financial arbitration.",
  },
  {
    id: "2",
    name: "Bob Smith",
    ...
    rating: 4.6,
    bio: "Specializes in tech-related disputes and IP conflicts.",
  },
  {
    id: "3",
    name: "Carol Williams",
    ...
    rating: 4.9,
    bio: "Expert in resolving workplace and employment disputes.",
  },
];

export const ChooseMediatorStep: React.FC<ChooseMediatorStepProps> = ({
  data,
  updateData,
}) => {
  const [searchTerm, setSearchTerm] = React.useState("");
  const [selectedMediator, setSelectedMediator] =
    React.useState<Mediator | null>(data);

  const filteredMediators = mockMediators.filter(
    (mediator) =>
      mediator.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      mediator.expertise.some((exp) =>
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-gray-800">
        Step 3: Select a Mediator or Arbitrator
      </h2>
      <p className="text-gray-600">
        Choose a neutral, qualified mediator from the platform.
      </p>
      ...
      <div className="grid gap-4 mt-4">
        {filteredMediators.map((mediator) => (
          <Card
            key={mediator.id}
            isPressable
            onPress={() => handleSelectMediator(mediator)}
          >
            <CardBody className="flex items-center space-x-4">
              ...
              <Button
                color="primary"
                variant={
                  selectedMediator?.id === mediator.id ? "solid" : "bordered"
                }
                size="sm"
                onClick={(e) => {
                  e.stopPropagation();
                  handleSelectMediator(mediator);
                }}
              >
      ...
      {filteredMediators.length === 0 && (
        <p className="text-center text-gray-500 mt-4">
          No mediators found. Please try a different search term or contact
          support.
        </p>
      )}
    </div>
